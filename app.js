/* ==========================================================================
   Clip — app.js
   A seven-day desk for phrases you reuse.
...
const CONFIG = {
  ...
  fileBase: 'clip',
  storageKey: 'clip.v1',
  cleanupTimeKey: 'clip.v1.lastCleanupAt',   // separate from storageKey — see saveState() note
  backupTimeKey: 'clip.v1.lastBackupAt',     // separate from storageKey — see saveState() note
  mirrorPrefix: 'clip-mirror-',
  mirrorUrl: './__clip-mirror.json',
  schema: 1,
  maxTextLength: 20000,
  maxLabelLength: 40,
  previewLong: 200,
...
let state = { version: CONFIG.schema, app: 'clip', items: [], deleted: [], settings: { ...CONFIG.defaults } };
let storageOK = true;
...
function normalize(data, opts = {}) {
  const out = { version: CONFIG.schema, app: 'clip', items: [], deleted: [], settings: { ...CONFIG.defaults } };
  if (!data || typeof data !== 'object') return out;

  if (Array.isArray(data.items)) {
    // Dump was removed (plan §6) — old dump entries are discarded, not migrated.
    out.items = data.items
      .filter(it => it && typeof it.text === 'string' && it.text.trim() !== '' && it.kind !== 'dump')
 ...
 * menu), edit+save, unpin, merge-on-recapture.
...
/**
 * Archives items to clip/archive/<YYYY-MM>.json (grouped by createdAt month,
...
// The Shortcut hands text over as a raw ?add= URL-encoded query param, built
// by iOS Shortcuts' own "URL Encode" + "Open URLs" actions — not by this
// app. iOS has a real (and inconsistent across versions) cap on how long a
// URL it will actually open, and when it truncates one it cuts through the
// percent-encoding itself, leaving a dangling "%" or "%A" at the very end.
// That is detectable even though the *original* length never was:
// URLSearchParams silently decodes a broken tail rather than throwing, so a
// truncated Shortcut clip would otherwise save with no sign anything was cut
// short — indistinguishable from a normal short clip. Flag it instead.
function rawQueryParam(search, key) {
  const match = new RegExp('[?&]' + key + '=([^&]*)').exec(search);
  return match ? match[1] : null;
}
function looksTruncatedByUrlLimit(rawValue) {
  return typeof rawValue === 'string' && /%[0-9A-Fa-f]?$/.test(rawValue);
}
...
function handleUrlIntake() {
  ...
  // ?paste=1 → Shortcuts can "Copy to Clipboard" then "Open URL" with this
  // short, fixed URL (no payload in the query string at all). The text
  // travels through the system clipboard instead of the URL, so it never
  // hits iOS's URL-length cap that truncates ?add= for long clips.
  ...
  const rawAdd = rawQueryParam(location.search, 'add');
  ...
  if (looksTruncatedByUrlLimit(rawAdd)) {
    toast('This clip may have been cut short by iOS’s URL length limit — for long text, open Clip and use Paste from clipboard instead.', 'warn', 7000);
  }
}
...
function cache() {
  [
    ...
    'mode-line', 'btn-pull-other', 'pull-other-hint', 'storage-line', 'backup-line', 'app-version-note',
    ...
    'journal-toggle', 'journal-content-toggle', 'journal-status-line', 'journal-error-line',
    'journal-from', 'journal-to', 'btn-journal-preview', 'btn-journal-import', 'btn-journal-redact', 'btn-journal-clear-activity', 'journal-backfill-status'
...
function daysBadge(item) {
  ...
  const label = shown === 1 ? '1 day left' : shown + ' days left';
  return '<span class="badge warn" aria-label="' + label + '">' + shown + 'd</span>';
}
...
function openMenu(item) {
  ...
  if (item.type === 'url') actions.push({ key: 'open', text: '↗ Open link' });
  if (item.type === 'email') actions.push({ key: 'mail', text: '✉ Compose email' });
  if (item.type === 'phone') actions.push({ key: 'tel', text: '☎ Call' });
...
function pasteShortcutUrl() { return baseUrl() + '?paste=1'; }
...
function refreshSettingsUI() {
  ...
  el['app-url'].textContent = pasteShortcutUrl();
  el['guide-url'].textContent = pasteShortcutUrl();
  el['app-version-note'].textContent = `Clip · App version ${APP_BUILD} · Data stays on this device unless Sync is on`;
  refreshCleanupLine();
...
async function exportJson() {
  const payload = JSON.stringify({
    version: CONFIG.schema, app: 'clip', savedAt: nowIso(),
    items: state.items, deleted: state.deleted, settings: state.settings,
    journalActivity: Object.values(readClipActivityLedger())
  }, null, 2);
...
function importJson(file) {
  ...
  reader.onload = async () => {
    ...
    if (!data || !Array.isArray(data.items)) { toast('This is not a Clip backup file.', 'err', 4000); return; }
    if (Number(data.version) > CONFIG.schema) { toast('This backup is from a newer version — update Clip first.', 'warn', 5000); return; }
...
/* ── 19. cross-device sync (plan 4-5, 5-1, 5-3) ──────────────────────────
   Writes full snapshots to webapp-data/clip/data.<contextId>.json, one file
...
const SYNC = {
  ...
  branch: 'main',
  dirPath: 'clip',
  basePath: 'clip/data.json',
  archiveDir: 'clip/archive',
  enabledKey: 'clip.syncEnabled',
  tokenKey: 'sync.token.v1',
  lastSyncKey: 'clip.lastSyncAt',
  debounceMs: 4000,
  tombstoneMaxDays: 90
};
...
async function pushNow() {
  ...
  try {
    await window.SharedSync.outboxEnqueueReplace(SYNC.namespace, { path, content: payload, message: 'sync: clip ' + stamp });
  } catch (e) {
...
async function initSync() {
  ...
  // iOS Home Screen apps are often suspended in place rather than reloaded, so
  // DOMContentLoaded only fires once at install/first open. Re-pull whenever the
  // app is foregrounded again (visibilitychange) or restored from bfcache
  // (pageshow persisted) so Home Screen stays in sync without a manual tap —
  // not just on a fresh full page load.
  ...
  window.addEventListener('pageshow', (e) => { if (e.persisted) pullAndMerge(); });
...
/* ── Journal projection (independent opt-in; never blocks Clip saves) ─── */

const JOURNAL = {
  enabledKey: 'clip.journalEnabled.v1',
  contentKey: 'clip.journalContent.v1',
  ledgerKey: 'clip.journalActivity.v1',
...
function isJournalContentEnabled() {
  try { return localStorage.getItem(JOURNAL.contentKey) !== '0'; } catch (e) { return true; }
}
function setJournalContentEnabled(value) {
  try { localStorage.setItem(JOURNAL.contentKey, value ? '1' : '0'); } catch (e) { /* ignore */ }
}
function journalTitle(item) {
  const firstLine = String(item.text || '').split('\n').find(line => line.trim()) || '';
  return String(item.label || '').trim() || firstLine.trim() || 'Untitled clip';
}
function clipJournalRecord(item, options = {}) {
  ...
  return {
    ...
    title: includeContent ? journalTitle(item) : 'Clip',
    data: {
      ...(includeContent ? { text: item.text, label: item.label } : {}),
      type: item.type,
...
function readClipActivityLedger() {
  try { const value = JSON.parse(localStorage.getItem(JOURNAL.ledgerKey) || '{}'); return value && typeof value === 'object' ? value : {}; } catch (e) { return {}; }
}
function writeClipActivityLedger(entries) {
  try { localStorage.setItem(JOURNAL.ledgerKey, JSON.stringify(entries)); } catch (e) { /* local activity must never block Clip */ }
}
function replaceClipActivityLedger(entries) {
  const ledger = {};
  const cutoff = Date.now() - 90 * 86400000;
  (Array.isArray(entries) ? entries : []).forEach(entry => {
    if (!entry || !validDateStamp(entry.date) || typeof entry.itemId !== 'string' || !entry.itemId) return;
    if (entry.itemType !== 'clip' || !Array.isArray(entry.actions)) return;
    if (!Number.isFinite(Date.parse(entry.firstAt)) || !Number.isFinite(Date.parse(entry.lastAt))) return;
    if (Date.parse(entry.lastAt) < cutoff) return;
    ledger[entry.date + ':' + entry.itemId] = {
      date: entry.date, itemId: entry.itemId, itemType: entry.itemType,
      sourceDate: validDateStamp(entry.sourceDate) ? entry.sourceDate : entry.date,
      actions: [...new Set(entry.actions.filter(action => typeof action === 'string'))],
      firstAt: entry.firstAt, lastAt: entry.lastAt
    };
  });
  writeClipActivityLedger(ledger);
  return ledger;
}
function clearClipActivityLedger() {
  try { localStorage.removeItem(JOURNAL.ledgerKey); return true; } catch (e) { return false; }
}
function withoutClipJournalContent(record) {
  if (record.kind === 'item-activity') {
    return { ...record, title: 'Clip', data: { ...(record.data || {}), contentIncluded: false } };
  }
  const { text, label, ...metadata } = record.data || {};
  return { ...record, title: 'Clip', data: { ...metadata, contentIncluded: false } };
}
function clipActivityRecord(entry) {
  const includeContent = isJournalContentEnabled();
  const item = state.items.find(candidate => candidate.id === entry.itemId);
  return { id: entry.itemId + ':' + entry.date, kind: 'item-activity', at: entry.firstAt, updatedAt: entry.lastAt,
    deleted: false, title: includeContent && item ? journalTitle(item) : 'Clip',
    data: { activityDate: entry.date, sourceDate: entry.sourceDate, itemType: 'Clip',
...
function recordClipActivity(item, action, previousSourceDate) {
  if (!item || !['copied', 'edited', 'pinned', 'unpinned', 'moved-to-today', 'exported'].includes(action)) return;
  const at = nowIso(); const date = localDateStamp(new Date()); const sourceDate = previousSourceDate || (validDateStamp(item.journalDate) ? item.journalDate : localDateStamp(item.createdAt));
  if (date === sourceDate && action !== 'moved-to-today') return;
  const ledger = readClipActivityLedger(); const key = date + ':' + item.id; const existing = ledger[key];
  ledger[key] = { date, itemId: item.id, itemType: item.kind, sourceDate, actions: [...new Set([...(existing?.actions || []), action])], firstAt: existing?.firstAt || at, lastAt: at };
  const cutoff = Date.now() - 90 * 86400000; Object.keys(ledger).forEach(id => { if (Date.parse(ledger[id].lastAt) < cutoff) delete ledger[id]; }); writeClipActivityLedger(ledger);
  if (isJournalEnabled()) queueClipActivity(ledger[key]);
}
function queueClipActivity(entry) {
  Promise.resolve().then(async () => { const module = await import(JOURNAL.moduleUrl); if (!module.JOURNAL_KINDS?.clip?.includes('item-activity')) { journalState = { ...journalState, status: 'error', errorCode: 'CONTRACT_STALE' }; refreshJournalUI(); return; } const client = await getJournalClient(); if (client) await client.enqueue(clipActivityRecord(entry), { date: entry.date }); }).catch(() => { journalState = { ...journalState, status: 'error', errorCode: 'QUEUE_FAILED' }; refreshJournalUI(); });
}
async function getJournalClient() {
  ...
  journalClientPromise = (async () => {
    ...
    return module.createJournalClient({
      app: 'clip', context, namespace: 'clip-journal',
...
function queueJournalItem(item, options = {}) {
  ...
  Promise.resolve().then(async () => {
    ...
    await client.enqueue(clipJournalRecord(item, options), {
...
async function refreshJournalUI() {
  ...
  if (el['journal-content-toggle']) el['journal-content-toggle'].checked = isJournalContentEnabled();
  const client = await getJournalClient();
  if (client) {
...
async function importJournalBackfill() {
  ...
  if (!ok) return;
  const client = await getJournalClient();
  if (!client) { toast('Journal module is unavailable. Clip data was not changed.', 'err'); return; }
  ...
  for (const item of preview.items) {
    await client.enqueue(clipJournalRecord(item), {
      date: validDateStamp(item.journalDate) ? item.journalDate : localDateStamp(item.createdAt)
    });
  }
...
async function redactJournalContent() {
  if (!isJournalEnabled()) { toast('Turn on Include in journal first.', 'warn'); return; }
  if (isJournalContentEnabled()) { toast('Turn off Upload content on every active installation first.', 'warn', 5000); return; }
  const from = el['journal-from'].value;
  const to = el['journal-to'].value;
  if (!backfillDayCount(from, to)) { toast('Choose a valid date range.', 'warn'); return; }
  const ok = await confirmAsk('Remove content from current Daybook records?',
    'This replaces this installation’s current projections with metadata-only revisions. Clip data and normal Sync stay unchanged. Git history is not erased.', 'Remove content');
  if (!ok) return;
  const client = await getJournalClient();
  if (!client?.redactRange) { toast('Update Shared and reload Clip first.', 'warn', 5000); return; }
  el['journal-backfill-status'].textContent = 'Removing content…';
  const result = await client.redactRange({ from, to, transform: withoutClipJournalContent });
  el['journal-backfill-status'].textContent = result.error
    ? `Partial · ${result.processedDates}/${result.totalDates} day(s) · pending work will retry`
...
function bind() {
  ...
  el['journal-toggle'].addEventListener('change', async () => {
    ...
    refreshJournalUI();
    toast(turningOn ? 'New Clip activity will be included in Daybook.' : 'Journal inclusion turned off.', 'ok', 3600);
  });
  el['journal-content-toggle'].addEventListener('change', async () => {
    setJournalContentEnabled(el['journal-content-toggle'].checked);
    const client = await getJournalClient();
    if (client && !isJournalContentEnabled()) await client.transformPending(record => ({ ...withoutClipJournalContent(record), updatedAt: nowIso() }));
    await reportJournalStatus();
  });
  ...
  el['btn-journal-redact'].addEventListener('click', redactJournalContent);
  el['btn-journal-clear-activity'].addEventListener('click', () => {
    if (!confirm('Clear the captured 90-day Journal activity history on this device? This does not delete Clip items or remote Journal records.')) return;
    clearClipActivityLedger();
    el['journal-backfill-status'].textContent = 'Captured activity cleared on this device.';
  });
