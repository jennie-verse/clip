...
const CACHE = 'clip-v1-2026.09.05-release1';
...
self.addEventListener('install', event => {
  event.waitUntil(
    caches.open(CACHE)
      .then(cache => Promise.all(ASSETS.map(url => fetch(new Request(url, { cache: 'reload' }))
  ...
  event.waitUntil(
    caches.keys()
      .then(keys => Promise.all(
        keys
          .filter(k => k.startsWith('clip-') && k !== CACHE && !k.startsWith(KEEP_PREFIX))
...
self.addEventListener('fetch', event => {
  ...
  if (url.pathname.indexOf('__clip-mirror') !== -1) return;

  // Navigations (including ?add= links) always resolve to index.html.
